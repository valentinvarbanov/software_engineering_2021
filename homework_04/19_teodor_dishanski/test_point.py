"""Test the logic in the Point class."""
...
def test_valid_point_instance() -> None:
    """Test for valid point instances."""
    point = Point(0.0, 0.0)
    assert isinstance(point, Point)
    assert point.x_coordinate == 0.0
    assert point.y_coordinate == 0.0
...
def test_invalid_point_instance() -> None:
    """Test for invalid point instances."""
    with pytest.raises(AssertionError):
        Point(0, 0)


def test_invalid_points_equality() -> None:
    """Test if two points are not equal."""
    with pytest.raises(AssertionError):
        assert Point(0.0, 1.0) == 0


def test_invalid_points_distance() -> None:
    """Test for egge cases in Point.distance() method"""
    with pytest.raises(AssertionError):
